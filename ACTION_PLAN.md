...
Hidralisk es la primera distribución Linux que resuelve la tensión histórica entre inmutabilidad y flexibilidad, entre declaratividad y compatibilidad, entre seguridad y usabilidad.

**Diferenciación clave:**
- Sistema inmutable por capas (ostree + overlayfs) compatible con paquetes `.deb`
- Overmind: orquestador declarativo YAML-based con firma obligatoria y schema validation
- Hidra-apt: gestor atómico con rollback vía Btrfs snapshots
...
|---|-------------------|------------|-----------|
| 1 | **dm-verity ausente** | Agregado como mandatory en stage-1 de `hidra-init`. Verificación a nivel de bloque antes de montar rootfs. | `ARCHITECTURE.md:79` |
...
| 4 | **`/var` y `/etc` zona libre** | Overmind como source of truth + verificación en boot de archivos protegidos contra manifest firmado. AppArmor profiles deniegan escritura directa. | `ARCHITECTURE.md:25-34`, `ARCHITECTURE.md:80-83` |
| 5 | **`hidra.yaml` sin firma ni schema** | JSON Schema validation en cada apply. Firma obligatoria. Campos críticos inmutables. Denylist de campos ejecutables. SRI para fuentes remotas. | `ARCHITECTURE.md:66-72` |
...
| V-03 | Supply Chain (hidra-apt) | ALTO | MEDIA | **ABIERTO** | **MITIGADO** | HSM/cosign + reproducible builds + sandbox apt |
| V-04 | Overlay Toxicity | ALTO | MEDIA | **ABIERTO** | **MITIGADO** | POST-mount hash verification + allowlist de capas |
| V-05 | Persistencia en /var, /etc | ALTO | ALTA | **ABIERTO** | **MITIGADO** | Overmind source of truth + boot verification + AppArmor |
| V-06 | Weaponización de Rollback | MEDIO-ALTO | BAJA | **ABIERTO** | **MITIGADO** | Rate limit + deprecated flag + pre-snapshot |
| V-07 | YAML Injection (Overmind) | CRÍTICO | MEDIA | **ABIERTO** | **MITIGADO** | Schema validation + firma + denylist + SRI |
...
| P1.3 | hidra-init stage-1 básico (verificación de hashes) | P1.2 | Core |
| P1.4 | hidra-apt MVP (apt wrapper + capa ostree) | P1.2 | Core |
| P1.5 | Overmind MVP (parseo YAML + apply de capas) | P1.2 | Core |
| P1.6 | ISO híbrida booteable | P1.1-P1.5 | Build |

**Entregable:** ISO funcional en QEMU/VirtualBox. hidra.yaml declarativo aplica paquetes.
...
| P2.4 | AppArmor profiles para hidra-init, hidra-apt, overmind | P1.3-P1.5 | Hardening |
...
| P2.10 | Overmind schema validation + firma + denylist | P1.5 | Core |
...
| P4.2 | hidra-apt con pinning de repositorios + hash verification | P1.4 | Core |
| P4.3 | Hidralisk ISO híbrida final con todas las defensas | P1-P3 | Build |
...
│           ├── P2.10 (schema + firma)
│           └── P2.6 (bubblewrap)
├── P1.5 (Overmind) ─── P2.3 (POST-mount) ─── P2.10 (schema)
...
| `ARCHITECTURE.md` | dm-verity en stage-1, TPM binding, POST-mount verification, /etc+/var defensas, schema validation, firma de hidra.yaml, protecciones de rollback, HSM para claves |
